...
static double __convert_none(double orig, double __attribute__((unused)) arg)
{
	return orig;
}
...
static const struct conv convs[] = {
	...
	{
		.sfx = (const char *[]){"inHG", "inHg", "inhg", NULL},
		.fromfn = __convert_divisor,
		.tofn = __convert_factor,
		.arg = 29.5300,
	},
	{
...
double parse_pressure(const char *str)
{
	...
	errx(1, "Cannot parse pressure '%s'", str);
}
...
double parse_time(const char *str)
{
	 ...
	 */
	if (seconds == 0.0)
		errx(1, "Bad time '%s': enter 'H:M', 'H:', ':M', or seconds", str);
...
void parse_arguments(int argc, char **argv, struct params *params)
{
	...
	while (1) {
		...
		case 'g':
			...
			if (params->gf <= 0.0)
				errx(1, "GF-Hi cannot be negative or zero");
		...
		case 'm':
			...
			if (!*cur)
				errx(1, "No match for model '%s'", optarg);
			break;
		case 'p':
			params->ppn2 = atof(optarg);
			if (params->ppn2 > 1.0)
				errx(1, "Can't have PPN2 more than 1.0");
			else if (params->ppn2 <= 0.0)
				errx(1, "Can't have negative (or zero) PPN2");
			break;
		case -1:
			goto done;
		case 'h':
	...
	if ((argc - optind) & 1)
		errx(1, "Bad input: missing a depth or time?");
